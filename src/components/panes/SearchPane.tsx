...
const SearchPane: React.FC<SearchPaneProps> = ({ rootPath }) => {
    ...
    const handleResultClick = useCallback(async (filePath: string) => {
        try {
    ...
    return (
        <div className="flex flex-col h-full p-4 overflow-hidden">
            <h2 className="text-xs font-bold text-[var(--text-secondary)] uppercase mb-4 tracking-wider">Search</h2>
            ...
            <div className="flex-1 overflow-y-auto min-h-0 space-y-2">
                {isSearching && <div className="flex justify-center py-4"><Loader2 className="animate-spin text-[var(--accent-primary)]" /></div>}

                {!isSearching && results.map((res, i) => (
                    <div key={i} className="group cursor-pointer hover:bg-white/5 p-2 rounded" onClick={() => handleResultClick(res.path)}>
                        <div className="text-xs text-[var(--accent-primary)] font-mono truncate" title={res.path}>
                            {res.path.split(/[\\/]/).pop()}
                        </div>
                        <div className="text-xs text-[var(--text-secondary)] mt-1 line-clamp-2 pl-2 border-l-2 border-[var(--border-secondary)]">
                            {res.preview}
                        </div>
                    </div>
                ))}
